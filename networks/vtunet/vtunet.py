...
class VTUNet(nn.Module):
    def __init__(self, args, num_classes=3, zero_head=False, embed_dim=96, win_size=7):
        ...
        self.swin_unet = SwinTransformerSys3D(img_size=(self.roi_x, self.roi_y, self.roi_z),
                                              patch_size=(4, 4, 4),
                                              in_chans=1,
    ...
    def load_from(self, args):
        pretrained_path = args.pretrained_weight
...
def window_partition(x, window_size):
    ...
    Args:
        x: (B, D, H, W, C)
        window_size (tuple[int]): window patch_size
...
class PositionalEncoding3D(nn.Module):
    ...
    def forward(self, tensor):
        """
        :param tensor: A 5d tensor of patch_size (batch_size, x, y, z, ch)
        :return: Positional Encoding Matrix of patch_size (batch_size, x, y, z, ch)
        """
...
class SwinTransformerBlock3D(nn.Module):
    ...
    Args:
        ...
        num_heads (int): Number of attention heads.
        window_size (tuple[int]): Window patch_size.
        shift_size (tuple[int]): Shift patch_size for SW-MSA.
    ...
    def forward(self, x, mask_matrix, prev_v, prev_k, prev_q, is_decoder=False):
        ...
        Args:
            x: Input feature, tensor patch_size (B, D, H, W, C).
...
class PatchExpand_Up(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.expand(x)
        B, L, C = x.shape
        # assert L == D * H * W, "input feature has wrong patch_size"

        x = x.view(B, D, H, W, C)
        x = rearrange(x, 'b d h w (p1 p2 c)-> b (d p1) (h p2) w c', p1=self.dim_scale, p2=self.dim_scale, c=C // 4)
...
class BasicLayer_up(nn.Module):
    ...
    Args:
        ...
        num_heads (int): Number of attention heads.
        window_size tuple(int): Local window patch_size.
        mlp_ratio (float): Ratio of mlp hidden dim to embedding dim.
        qkv_bias (bool, optional): If True, add a learnable bias to query, key, value. Default: True
        qk_scale (float | None, optional): Override default qk scale of head_dim ** -0.5 if set.
        drop (float, optional): Dropout rate. Default: 0.0
    ...
    def forward(self, x, prev_v1, prev_k1, prev_q1, prev_v2, prev_k2, prev_q2):
        ...
        Args:
            x: Input feature, tensor patch_size (B, C, D, H, W).
        """
...
class BasicLayer(nn.Module):
    ...
    Args:
        ...
        window_size (tuple[int]): Local window patch_size. Default: (1,7,7).
...
class PatchEmbed3D(nn.Module):
    ...
    Args:
        patch_size (int): Patch token patch_size. Default: (2,4,4).
...
class SwinTransformerSys3D(nn.Module):
    ...
    Args:
        img_size (int | tuple(int)): Input image patch_size. Default 224
        patch_size (int | tuple(int)): Patch patch_size. Default: 4
        in_chans (int): Number of input image channels. Default: 3
        num_classes (int): Number of classes for classification head. Default: 1000
        embed_dim (int): Patch embedding dimension. Default: 96
        depths (tuple(int)): Depth of each Swin Transformer layer.
        num_heads (tuple(int)): Number of attention heads in different layers.
        window_size (tuple(int)): Window patch_size. Default: (7,7,7)
        mlp_ratio (float): Ratio of mlp hidden dim to embedding dim. Default: 4
