...
class SwinTransformerBlock_kv(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, relative_mult=True):
        super().__init__()
        self.dim = dim
        ...
        self.attn = WindowAttention_kv(
            ...
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, relative_mult=relative_mult)
    ...
    def forward(self, x, mask_matrix, skip=None, x_up=None):
        ...
        assert L == S * H * W, "input feature has wrong patch_size"
        ...
        # pad feature maps to multiples of window patch_size
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        if tuple(self.input_resolution) == tuple(self.window_size):
            # if window patch_size is larger than input resolution, we don't partition windows
            self.shift_size = [0, 0, 0]
    ...
    def forward(self, x, mask_matrix):
        ...
        Args:
            x: Input feature, tensor patch_size (B, H*W, C).
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, dim, norm_layer=nn.LayerNorm, tag=None):
        ...
        if tag == 0:
            ...
            self.reduction = nn.Conv3d(dim, dim * 2, kernel_size=[3, 3, 1], stride=[2, 2, 1], padding=[1, 1, 0])
        elif tag == 1:
            self.reduction = nn.Conv3d(dim, dim * 2, kernel_size=[3, 3, 3], stride=[2, 2, 2], padding=[1, 1, 1])
        else:
            # self.reduction = nn.Conv3d(dim, dim*2,kernel_size=[3,3,3],stride=[2,2,2],padding=[1,1,0])
            self.reduction = nn.Conv3d(dim, dim * 2, kernel_size=[3, 3, 3], stride=[2, 2, 2], padding=[1, 1, 1])  # Mine
    ...
    def forward(self, x, S, H, W):
        ...
        assert L == H * W * S, "input feature has wrong patch_size"
        x = x.view(B, S, H, W, C)
...
class Patch_Expanding(nn.Module):
    def __init__(self, dim, norm_layer=nn.LayerNorm, tag=None):
        ...
        if tag == 0:
            ...
            self.up = nn.ConvTranspose3d(dim, dim // 2, [2, 2, 1], [2, 2, 1])  # mine
        elif tag == 1:
            self.up = nn.ConvTranspose3d(dim, dim // 2, [2, 2, 2], [2, 2, 2])
        elif tag == 2:
            # self.up=nn.ConvTranspose3d(dim,dim//2,[2,2,2],[2,2,2],output_padding=[1,0,0]) base
            self.up = nn.ConvTranspose3d(dim, dim // 2, [2, 2, 2], [2, 2, 2], output_padding=[0, 0, 0])  # mine
            # self.up=nn.ConvTranspose3d(dim,dim//2,[2,2,2],[2,2,2],output_padding=[0,0,1])
    ...
    def forward(self, x, S, H, W):
        ...
        if self.downsample is not None:
            ...
            if self.i_layer != 1 and self.i_layer != 2:
                ...
                Ws, Wh, Ww = (S + 1) // 2, (H + 1) // 2, W
            else:
                if W != 1:
                    Ws, Wh, Ww = (S + 1) // 2, (H + 1) // 2, W // 2
                else:
                    Ws, Wh, Ww = (S + 1) // 2, (H + 1) // 2, W
...
class BasicLayer_up(nn.Module):
    ...
    def forward(self, x, skip, S, H, W):
        ...
        x = skip + x_up
        if self.i_layer == 1 or self.i_layer == 0:
            if W == 1:
                S, H, W = S * 2, H * 2, W
            else:
                S, H, W = S * 2, H * 2, W * 2
        # elif self.i_layer==0:
        # S, H, W = (S * 2)+1 , H * 2, W * 2
        # S, H, W = S * 2 , H * 2, W * 2 #mine
        # S, H, W = S * 2, H * 2, (W * 2) + 1  # mine
        else:
            # S, H, W = S , H * 2, W * 2
            S, H, W = S * 2, H * 2, W
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=4, embed_dim=96, norm_layer=None):
        ...
        stride1 = [patch_size[0] // 2, patch_size[1] // 2, 1]  # mine
        stride2 = [patch_size[0] // 2, patch_size[1] // 2, 1]
        # stride1 = [1, patch_size[1] // 2, patch_size[2] // 2] #classic
...
class nnFormer(SegmentationNetwork):
    ...
    def __init__(self, crop_size=[64, 128, 128],
        ...
        self.model_down = Encoder(pretrain_img_size=crop_size, window_size=window_size, embed_dim=embed_dim,
                                  ...
                                  down_stride=down_stride, relative_mult=relative_mult)
        self.decoder = Decoder(pretrain_img_size=crop_size, embed_dim=embed_dim, window_size=window_size[::-1][1:],
                               ...
                               up_stride=down_stride[::-1][1:], relative_mult=relative_mult)
    ...
    def forward(self, x):
        ...
        if self.do_ds:
            ...
            if seg_outputs_bbox is not None:
                return seg_outputs[::-1], seg_outputs_bbox[-1]
            else:
