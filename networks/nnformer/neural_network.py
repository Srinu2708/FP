...
from utils.utils import no_op
from utils.utils import to_cuda, maybe_to_torch
...
class SegmentationNetwork(NeuralNetwork):
    def __init__(self):
        ...
        # if we have 5 pooling then our patch patch_size must be divisible by 2**5
    ...
    def predict_3D(self, x: np.ndarray, do_mirroring: bool, mirror_axes: Tuple[int, ...] = (0, 1, 2),
        ...
        :param step_size: When running sliding window prediction, the step patch_size determines the distance between adjacent
        predictions. The smaller the step patch_size, the denser the predictions (and the longer it takes!). Step patch_size is given
        as a fraction of the patch_size. 0.5 is the default and means that wen advance by patch_size * 0.5 between
        predictions. step_size cannot be larger than 1!
        :param patch_size: The patch patch_size that was used for training the network. Do not use different patch sizes here,
    ...
    def _compute_steps_for_sliding_window(patch_size: Tuple[int, ...], image_size: Tuple[int, ...], step_size: float) -> \
        ...
        assert [i >= j for i, j in
                zip(image_size, patch_size)], "image patch_size must be as large or larger than patch_size"
        assert 0 < step_size <= 1, 'step_size must be larger than 0 and smaller or equal to 1'

        # our step width is patch_size*step_size at most, but can be narrower. For example if we have image patch_size of
        # 110, patch patch_size of 64 and step_size of 0.5, then we want to make 3 steps starting at coordinate 0, 23, 46
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        # for sliding window inference the image must at least be as large as the patch patch_size. It does not matter
        # whether the shape is divisible by 2**num_pool as long as the patch patch_size is
        ...
        if verbose:
            ...
            print("patch patch_size:", patch_size)
        ...
        # we reverse the padding here (remeber that we padded the input to be at least as large as the patch patch_size
        slicer = tuple(
