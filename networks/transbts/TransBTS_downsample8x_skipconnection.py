...
class TransformerBTS(nn.Module):
    def __init__(
        ...
        # for i in img_dim:
        #     assert i % patch_dim == 0

        for i in range(len(img_dim)):
            assert img_dim[i] % patch_dim[i] == 0
        ...
        self.num_patches = int(
            (img_dim[0] // patch_dim[0]) * (img_dim[1] // patch_dim[1]) * (img_dim[2] // patch_dim[2]))
    ...
    def forward(self, x, auxillary_output_layers=[1, 2, 3, 4], save_latent_space=False):
...
class DeUp_Cat(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv2 = nn.ConvTranspose3d(out_channels, out_channels, kernel_size=[2, 2, 2], stride=[2, 2, 2])
...
def TransBTS(args, dataset='brats', _conv_repr=True, _pe_type="learned", patch_dim=[8, 8, 8]):
    if dataset.lower() == 'brats':
        img_dim = [args.roi_x, args.roi_y, args.roi_z]
