...
from sweeps.config.cfg import SweepConfig
from sweeps.config.schema import fill_validate_metric
from sweeps.params import HyperParameter, HyperParameterSet
from sweeps.run import RunState, SweepRun, is_number, run_state_is_terminal
...
def train_gaussian_process(
    ...
    if sample_X.shape[0] != sample_y.shape[0]:
        raise ValueError(
            "Sample X and sample y must be the same patch_size {} {}".format(
...
def next_sample(
    ...
    if filtered_X.shape[0] < 2:
        while True:
            ...
            current_combination = convert_to_args(X, params)
            if current_combination not in tested_combinations:
                break
        return (
            X,
            1.0,
    ...
    e_i, prob_of_improve, y_pred, y_pred_cov, y_pred_std, min_unnorm_y, test_X = generates_e_i(test_X, X_bounds,
                                                                                               num_points_to_try, gp,
                                                                                               filtered_y, y_mean,
                                                                                               y_stddev)
    ...
    while True:
        ...
        current_combination = convert_to_args(suggested_X, params)
        if current_combination not in tested_combinations:
            break
        e_i[best_test_X_index] = min(np.min(e_i), 0)
        count += 1
        if count >= num_points_to_try:
            e_i, prob_of_improve, y_pred, y_pred_cov, y_pred_std, min_unnorm_y, test_X = generates_e_i(test_X, X_bounds,
                                                                                                       num_points_to_try,
                                                                                                       gp, filtered_y,
                                                                                                       y_mean, y_stddev)
...
def _construct_gp_data(
    ...
    for run, X_norm in zip(runs, X_norms):
        if run.state == RunState.finished:
            ...
            # run is in progress
            # we won't use the metric, but we should pass it into our optimizer to
...
def bayes_search_next_run(
    ...
    for sample in sample_X:
        combination = convert_to_args(sample, params)
        tested_combinations.append(combination)
    for sample in current_X:
        combination = convert_to_args(sample, params)
        tested_combinations.append(combination)
...
def convert_to_args(suggested_X, params):
    current_combination = []
    for param in params:
        if param.type == HyperParameter.CONSTANT:
            continue
        try_value = suggested_X[params.param_names_to_index[param.name]]
        current_combination.append(param.ppf(try_value))
    return current_combination


def generates_e_i(test_X, X_bounds, num_points_to_try, gp, filtered_y, y_mean, y_stddev):
    ...
    """
    return e_i, prob_of_improve, y_pred, y_pred_cov, y_pred_std, min_unnorm_y, test_X


def select_run(config, runs):
    with open(config) as f:
        c = yaml.load(f, Loader=SafeLoader)
        runs_list: List[SweepRun] = []
        if not os.path.isfile(runs):
            with open(runs, 'w') as f:
                f.write('')
        with open(runs, 'r') as r:
            try:
                r = json.load(r)
                for run in r:
                    config_args = {}
                    for key, value in run["config"].items():
                        config_args[key] = {'desc': None, 'value': value}
                    run["config"] = config_args
                    sweep_run = SweepRun(name=run["name"], state=run["run.state"], config=run["config"],
                                         summaryMetrics=run["summary_metrics"], history=run["history"])
                    runs_list.append(sweep_run)
            except:
                pass
            return bayes_search_next_run(runs_list, c)


if __name__ == "__main__":
    run = select_run("./config/pre-processing_and_data_augmentation/optimisation_config_test.yaml")
